...
MAIN
  --DISPLAY "SLAVE:", fgl_getenv("SLAVE"),",VERBOSE:",fgl_getenv("VERBOSE")
...
FUNCTION createXCF(appfile,module,args,invokeShell)
  ...
  END FOR
  IF m_gasversion<3.2 AND module.getCharAt(1)=="/" THEN --we were invoked via absolute path
...
FUNCTION myerr(err)
  DEFINE err STRING
  DISPLAY "ERROR:",err
  CALL writeStderr(sfmt("ERROR:%1",err))
  EXIT PROGRAM 1
END FUNCTION

FUNCTION writeStderr(logstr STRING)
  DEFINE ch base.Channel
  LET ch = base.Channel.create()
  CALL ch.openFile("<stderr>", "w")
  CALL ch.writeLine(logstr)
  CALL ch.close()
END FUNCTION
...
FUNCTION openBrowser()
  DEFINE url,cmd,browser,lbrowser,pre,gdcm STRING
